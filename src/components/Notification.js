...
function Notification() {
  const { setIsNoticationOpen, currentColor } = useStateContext();
  return (
    <div
    ...
    >
      <div className="flex justify-between">
        <div className="flex items-center">
          <p className="font-bold text-xl dark:text-gray-300">Notification</p>
      ...
      <div className="">
        {chatData.map((item, index) => {
          return (
            <div
              key={index}
              className="flex justify-start mt-2 p-3 gap-5 cursor-pointer"
            >
              <img
                src={item.image}
